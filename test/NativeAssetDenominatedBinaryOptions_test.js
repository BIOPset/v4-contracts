...
contract("NativeAssetDenominatedBinaryOptions", (accounts) => {
    ...
    return NativeAssetDenominatedBinaryOptions.deployed().then(async function (
      ...
      return FakePriceProvider.deployed().then(async function (pp) {
        return UtilizationRewards.deployed().then(async function (ur) {
          ...
          var stakinBonus = await ur.getLPStakingBonus(
            lastStakeTime,
            lastInterchange,
            totalInterchange,
            stakedAmount,
          ...
          var stakinBonus2 = await ur.getLPStakingBonus(
            lastStakeTime2,
            lastInterchange2,
            totalInterchange2,
            stakedAmount2,
  ...
  it("exercise an call option", () => {
    return NativeAssetDenominatedBinaryOptions.deployed().then(async function (
      ...
      return FakePriceProvider.deployed().then(async function (pp) {
        ...
        var pendingClaims2 = await bo.getPendingClaims(accounts[2]);
        console.log(
          `pending claims after exercise ${web3.utils.fromWei(
            pendingClaims2,
            "ether"
          )}.`
        );
        ...
        console.log(
          `eth bo balance ${ethPoolB1}, 2 ${ethPoolB2}. ${
            ethPoolB1 == toWei(9)
          }`
        );
  ...
  it("exercise an put option", () => {
    return NativeAssetDenominatedBinaryOptions.deployed().then(async function (
      ...
      return FakePriceProvider.deployed().then(async function (pp) {
        ...
        var pendingClaims = await bo.getPendingClaims(accounts[2]);
        console.log(
          `pending claims ${web3.utils.fromWei(pendingClaims, "ether")}`
        );
        await pp.setPrice(basePrice - 10);
        await bo.complete(1);
        var ethB2 = await web3.eth.getBalance(accounts[2]);
  ...
  it("after actions, BIOP balance > 0", () => {
    return NativeAssetDenominatedBinaryOptions.deployed().then(async function (
      ...
      console.log(
        `last Withdraw ${lastWithdraw} block #1 ${blockNumber} #2 ${blockNumber2}`
      );
      console.log(
        ` biop pending claim ${web3.utils.fromWei(
          balance,
          "ether"
        )}, 2 ${web3.utils.fromWei(balance2, "ether")}`
      );
  ...
  it("can't withdraw more then they deposited", () => {
    return NativeAssetDenominatedBinaryOptions.deployed().then(async function (
      ...
      try {
        await bo.withdraw(toWei(10), { from: accounts[6] });
        assert.equal(true, false, "should never reach this line");
      } catch (e) {
        assert.equal(true, true, "test should have thrown");
      }
